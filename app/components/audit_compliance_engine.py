...
class ComplianceReportGenerator:
    ...
    def _collect_metrics(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        events = [e for e in self.audit_manager.events if start_ts <= e.timestamp <= end_ts]
        violations = [v for v in self.audit_manager.violations if start_ts <= v.detected_at <= end_ts]
...
class CertificateManager:
    ...
    def _save_certificates(self):
        ...
        try:
            ...
            data = {
                "certificates": [asdict(c) for c in self.certificates[-1000:]]
            }
...
def render_compliance_dashboard(engine: Dict):
    ...
    if stats["violation_distribution"]:
        ...
        fig.add_trace(
            go.Pie(labels=violations, values=counts),
            row=1, col=2
        )
        ...
        df = pd.DataFrame([
            {
                ...
                "Description": e.description[:50] + "..." if len(e.description) > 50 else e.description
            }
            for e in events
        ])
...
def render_violation_management(engine: Dict):
    ...
    with col1:
        policy_filter = st.selectbox(
            "Policy Type",
            ["All"] + [p.value for p in PolicyType]
        )
    with col2:
        severity_filter = st.selectbox(
            "Severity",
    ...
    if violations:
        for violation in violations:
            with st.expander(
                ...
                expanded=violation.severity in [AuditSeverity.CRITICAL, AuditSeverity.HIGH]
            ):
                col1, col2 = st.columns([3, 1])
                with col1:
                    st.markdown(f"**Description:** {violation.description}")
                    st.caption(f"Detected: {datetime.fromtimestamp(violation.detected_at).strftime('%Y-%m-%d %H:%M')}")
...
def render_compliance_reports(engine: Dict):
    ...
    with st.expander("📝 Generate New Report", expanded=False):
        ...
        with col1:
            ...
            start_date = st.date_input("Period Start", datetime.now() - timedelta(days=30))
        with col2:
    ...
    if hasattr(st.session_state, "last_report"):
        ...
        if report.findings:
            ...
            for finding in report.findings:
                st.caption(f"• {finding['description']} (Severity: {finding.get('severity', 'info')})")
...
def render_certificate_management(engine: Dict):
    ...
    with st.expander("📜 Issue New Certificate", expanded=False):
        col1, col2 = st.columns(2)
        with col1:
            cert_title = st.text_input("Certificate Title", "Academic Integrity Compliance")
            recipient = st.text_input("Recipient")
        with col2:
            regulation = st.selectbox("Regulation", [r.value for r in Regulation])
            issued_by = st.text_input("Issued By", st.session_state.get("username", "system"))
        
        expires = st.checkbox("Add Expiration")
        expires_at = None
        if expires:
            expires_at = st.date_input("Expiration Date", datetime.now() + timedelta(days=365))
    ...
    if cert_manager.certificates:
        ...
        if selected_cert:
            idx = [f"{c.title} - {c.recipient}" for c in cert_manager.certificates].index(selected_cert)
            ...
            with st.expander("📄 Certificate Details", expanded=True):
                ...
                st.markdown(f"**Issued:** {datetime.fromtimestamp(cert.issued_at).strftime('%Y-%m-%d %H:%M')}")
                if cert.expires_at:
                    st.markdown(f"**Expires:** {datetime.fromtimestamp(cert.expires_at).strftime('%Y-%m-%d')}")
                st.markdown(f"**Certificate Hash:** `{cert.certificate_hash}`")
                
                if st.button("Revoke Certificate", type="secondary", use_container_width=True):
