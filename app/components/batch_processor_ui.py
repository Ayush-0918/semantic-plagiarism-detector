    ...
    if uploaded_files:
        ...
        if st.button("🚀 Start Batch Processing", type="primary", use_container_width=True):
            ...
            with st.spinner(f"Processing {len(file_bytes_dict)} documents..."):
                ...
                if job:
                    st.success(f"✅ Processing complete! {job.processed_files}/{job.total_files} successful")
...
def render_job_progress(job: BatchJob) -> None:
    ...
    if job.processed_files > 0:
        ...
        # Success rate
        success_rate = (len(job.results) / job.processed_files) * 100 if job.processed_files > 0 else 0
        ...
        if jobs:
            ...
            for job_id, job in jobs.items():
                job_data.append({
                    ...
                    "Duration": f"{job.get_duration():.1f}s" if job.get_duration() else "-",
