...
class NotificationManager:
        ...
        try:
            ...
            if data_path.exists():
                with open(data_path, 'r') as f:
                    ...
                    self.alert_rules = [
                        AlertRule(**r) for r in data.get("rules", [])
                    ]
    ...
    def _send_in_app(self, notification: Notification):
        ...
        if len(st.session_state.in_app_notifications) > 100:
            st.session_state.in_app_notifications = st.session_state.in_app_notifications[-100:]
...
def render_notification_center():
    ...
    if "notification_manager" not in st.session_state:
        ...
        st.session_state.notification_manager = NotificationManager(data_dir / "notifications")
...
def render_notification_list(manager: NotificationManager, user: str, include_read: bool = True):
        ...
        with st.expander(
            ...
            with col1:
                ...
                if notification.actions:
                    ...
                    for col, action in zip(cols, notification.actions):
                        if col.button(action.get("label", "Action"), key=f"notif_{notification.id}_{action.get('id', '')}"):
                            # Handle action
                            pass
                
                st.caption(f"Priority: {notification.priority.name} | Channel: {notification.channel.value}")
            ...
            with col2:
                ...
                if notification.read_at:
                    st.caption(f"Read: {datetime.fromtimestamp(notification.read_at).strftime('%H:%M:%S')}")
...
def render_notification_settings(manager: NotificationManager, user: str):
    ...
    with st.form("notification_settings_form"):
        ...
        with col1:
            quiet_start = st.number_input("Quiet Hours Start", 0, 23, prefs.quiet_hours_start or 22)
        with col2:
            quiet_end = st.number_input("Quiet Hours End", 0, 23, prefs.quiet_hours_end or 6)
        
        quiet_enabled = st.checkbox("Enable Quiet Hours", value=prefs.quiet_hours_start is not None)
        
        st.markdown("##### Muted Keywords")
        muted_keywords = st.text_area(
            "Muted Keywords (one per line)",
        ...
        if st.form_submit_button("💾 Save Settings", use_container_width=True):
            ...
            updated_prefs = UserNotificationPreferences(
                ...
                muted_keywords=[k.strip() for k in muted_keywords.split('\n') if k.strip()],
