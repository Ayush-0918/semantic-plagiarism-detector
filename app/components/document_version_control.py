...
class VersionManager:
    ...
    def rollback(self, document_id: str, version_id: str, author: str) -> Optional[DocumentVersion]:
    ...
    def compare_versions(
        ...
        change_percentage = (total_changes / total_lines * 100) if total_lines > 0 else 0
    ...
    def get_change_summary(self, document_id: str, version_id: str) -> ChangeSummary:
        ...
        versions = self.versions.get(document_id, [])
        idx = next((i for i, v in enumerate(versions) if v.version_id == version_id), -1)
        ...
        return ChangeSummary(
            ...
            word_change=abs(len(version.content.split()) - len(prev_version.content.split())),
    ...
    def get_evolution_timeline(self, document_id: str) -> Dict[str, Any]:
        ...
        for version in versions:
            timeline.append({
                ...
                "date": datetime.fromtimestamp(version.timestamp).strftime("%Y-%m-%d"),
                "size": version.size,
        ...
        return {
            ...
            "growth_rate": (timeline[-1]["size"] - timeline[0]["size"]) / max(timespan, 1),
...
class ChangeTracker:
    ...
    def analyze_editing_patterns(self, document_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "edits_per_day": (len(versions) - 1) / ((versions[-1].timestamp - versions[0].timestamp) / 86400) if versions else 0
        }
...
def render_version_history(version_manager: VersionManager, doc_id: str, doc_name: str):
    ...
    for version in reversed(versions):
        version_data.append({
            ...
            "Date": datetime.fromtimestamp(version.timestamp).strftime("%Y-%m-%d %H:%M"),
            "Author": version.author,
            "Change Type": version.change_type,
            "Words": f"{version.word_count:,}",
            "Size": f"{version.size:,}",
            "Comment": version.comment[:50] + "..." if len(version.comment) > 50 else version.comment
        })
    ...
    if selected_version:
        ...
        if selected:
            ...
            with col1:
                st.text_area(
                    "Content",
                    selected.content[:500] + "..." if len(selected.content) > 500 else selected.content,
                    height=200,
                    disabled=True
                )
            with col2:
                ...
                # Change summary
                summary = version_manager.get_change_summary(doc_id, selected.version_id)
                if summary:
...
def render_version_comparison(version_manager: VersionManager, doc_id: str):
    ...
    if version_a and version_b:
        ...
        if v_a and v_b:
            # Compare
            diff = version_manager.compare_versions(doc_id, v_a.version_id, v_b.version_id)
...
def render_evolution_analytics(version_manager: VersionManager, doc_id: str):
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=("Size Evolution", "Word Count", "Edit Frequency", "Author Contributions")
    )
    ...
    if len(df) > 1:
        ...
        fig.add_trace(
            go.Bar(x=freq.index, y=freq.values, name="Edits"),
            row=2, col=1
        )
    ...
    if active_periods:
        ...
        for period in active_periods:
            st.caption(f"• {period['count']} edits in {((period['end'] - period['start']) / 60):.1f} minutes")
    else:
    ...
    if selected:
        ...
        if target:
            ...
            st.text_area(
                ...
                target.content[:500] + "..." if len(target.content) > 500 else target.content,
                height=150,
                disabled=True
            )
            ...
            if st.button("⏪ Execute Rollback", type="primary", use_container_width=True):
                with st.spinner("Rolling back..."):
                    new_version = version_manager.rollback(doc_id, target.version_id, author)
                    
                    if new_version:
                        # Update document content in session state
                        st.session_state[f"doc_content_{doc_id}"] = target.content
                        
                        st.success(f"✅ Successfully rolled back to version {version_num}")
