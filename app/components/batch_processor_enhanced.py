...
class EnhancedBatchProcessor:
    ...
    def get_batch_analytics(self) -> Dict[str, Any]:
        ...
        # Calculate metrics
        total_time = sum(
            (j.completed_at or 0) - (j.started_at or 0)
            for j in self.completed_jobs
        )
        
        success_rate = len([j for j in self.completed_jobs if j.status == JobStatus.COMPLETED]) / len(self.completed_jobs)
        
        return {
            "total_batches": len(self.completed_jobs),
            "avg_processing_time": total_time / len(self.completed_jobs) if self.completed_jobs else 0,
...
class BatchScheduler:
    ...
    def _load_schedules(self):
        ...
        try:
            schedule_path = Path(st.session_state.get("data_dir", ".")) / "batch_schedules.json"
...
def render_batch_processor_ui():
    ...
    with col3:
        if st.button("📊 Stats", use_container_width=True):
            st.session_state.show_batch_stats = not st.session_state.get("show_batch_stats", False)
    ...
    if st.session_state.get("show_batch_stats", False):
        ...
        # Progress bar
        st.progress(stats["progress"], text=f"Overall Progress: {stats['progress']:.1%}")
...
def render_batch_analytics():
    ...
    if processor.completed_jobs:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=("Processing Times", "Success/Fail", "Job Duration Distribution", "Retry Counts")
        )
        
        # Processing times
        times = [(j.completed_at or 0) - (j.started_at or 0) for j in jobs]
        fig.add_trace(
            go.Bar(y=times, name="Time", marker_color="blue"),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Histogram(x=durations, nbinsx=20),
            row=2, col=1
        )
        ...
        fig.add_trace(
            go.Bar(x=list(range(max(retries) + 1)), 
