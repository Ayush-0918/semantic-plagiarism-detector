...
class WorkflowEngine:
    ...
    def _process_task(self, workflow_id: str, task_id: str):
        ...
        try:
            ...
            if handler:
                ...
                raise ValueError(f"No handler for function: {task_data.get('function')}")
    ...
    def _handle_approval(self, task: Dict, workflow: Workflow) -> Dict:
        ...
        while time.time() - start_time < timeout:
            ...
            if approval and approval.status in [WorkflowStatus.APPROVED, WorkflowStatus.REJECTED]:
                break
            time.sleep(5)
    ...
    def cancel_workflow(self, workflow_id: str):
        """Cancel a workflow."""
        workflow = self.workflows.get(workflow_id)
        if workflow and workflow.status in [WorkflowStatus.PENDING, WorkflowStatus.RUNNING, WorkflowStatus.PAUSED]:
    ...
    def _calculate_progress(self, workflow: Workflow) -> float:
        ...
        completed_tasks = len([
            ...
            if t.get("status") in [TaskStatus.COMPLETED.value, TaskStatus.SKIPPED.value]
        ])
    ...
    def get_workflow_stats(self) -> Dict[str, Any]:
        ...
        pending_approvals = len([a for a in self.approvals if a.status == WorkflowStatus.APPROVAL_PENDING])
...
def render_workflow_dashboard(engine: WorkflowEngine):
    ...
    fig = make_subplots(rows=1, cols=2, subplot_titles=("Workflow Status", "Approval Status"))
    ...
    status_counts = {
        ...
        "failed": stats["failed"],
        "pending": stats["total_workflows"] - stats["running"] - stats["completed"] - stats["failed"]
    }
...
def render_workflow_list(engine: WorkflowEngine):
    ...
    with col1:
        status_filter = st.selectbox(
            "Filter by Status",
            ["All"] + [s.value for s in WorkflowStatus]
        )
    with col2:
        category_filter = st.selectbox(
            "Filter by Category",
            ["All"] + [c.value for c in WorkflowCategory]
        )
    ...
    for workflow in workflows:
        ...
        with st.expander(
            ...
            with col1:
                ...
                st.caption(f"Created: {datetime.fromtimestamp(workflow.created_at).strftime('%Y-%m-%d %H:%M')}")
            ...
            if workflow.tasks:
                ...
                for task in workflow.tasks:
                    ...
                    st.caption(f"{icon} {task.get('name')} - {task.get('status', 'unknown')}")
...
def render_template_management(engine: WorkflowEngine):
    ...
    if hasattr(st.session_state, "template_to_use"):
        ...
        if template:
            ...
            col1, col2 = st.columns(2)
            with col1:
                name = st.text_input("Workflow Name", f"{template.name} - {datetime.now().strftime('%Y-%m-%d')}")
            with col2:
                created_by = st.text_input("Created By", st.session_state.get("username", "system"))
    ...
    for approval in filtered[-20:]:
        ...
        with st.expander(
            ...
            with col1:
                ...
                st.caption(f"Created: {datetime.fromtimestamp(approval.created_at).strftime('%Y-%m-%d %H:%M')}")
...
def render_workflow_creation(engine: WorkflowEngine):
    ...
    with st.form("create_workflow_form"):
        ...
        with col2:
            description = st.text_area("Description", "Automated plagiarism detection workflow")
        ...
        trigger_config = st.text_area("Trigger Configuration (JSON)", '{"schedule": "daily"}')
