...
def render_hybrid_similarity_metrics(
    ...
    with col4:
        ...
        if semantic_score > lexical_score:
            st.metric("📊 Dominant", "🧠 Semantic", delta=f"+{semantic_score - lexical_score:.1%}")
        elif lexical_score > semantic_score:
            st.metric("📊 Dominant", "📝 Lexical", delta=f"+{lexical_score - semantic_score:.1%}")
        else:
...
def render_hybrid_stats_chart(hybrid_stats: Dict[str, Any]) -> None:
    ...
    if hybrid_stats['semantic_avg'] > 0:
        improvement = (hybrid_stats['hybrid_avg'] - hybrid_stats['semantic_avg']) / hybrid_stats['semantic_avg']
...
def compute_and_display_hybrid_scores(
    ...
    if scale_scores:
        lexical_matrix = scale_lexical_matrix(lexical_matrix, steepness=steepness, midpoint=midpoint)
    ...
    stats = {
        ...
        'lexical_avg': sum(lexical_scores) / len(lexical_scores) if lexical_scores else 0,
