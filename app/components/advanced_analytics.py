...
from src.utils.redis_cache import cache_session_state, get_session_state
    ...
    def _load_stopwords(self, language: str) -> Set[str]:
        ...
        stopwords_sets = {
            ...
            "es": {"el", "la", "los", "las", "un", "una", "unos", "unas", "de", "del", 
                   "en", "por", "para", "con", "sin", "sobre", "tras", "durante", 
                   "mediante", "entre", "hasta", "desde", "hacia", "a", "ante", "bajo", 
                   "cabe", "contra", "según"},
            "fr": {"le", "la", "les", "un", "une", "des", "du", "de", "à", "en", "pour", 
                   "par", "avec", "sans", "sur", "sous", "entre", "dans", "contre", 
                   "après", "avant", "pendant", "depuis", "jusque", "chez"},
        }
...
class ContextPreservingChunker:
    ...
    def chunk_with_context(
        ...
        for i, sentence in enumerate(sentences):
            ...
            if sentence_size > self.chunk_size:
                if current_chunk:
                    ...
                    chunks.append((
                        chunk_text,
                        self._create_chunk_metadata(current_chunk, i - len(current_chunk), i)
                    ))
                    current_chunk = []
                    current_size = 0
                ...
                for sub_chunk in sub_chunks:
                    chunks.append((
                        sub_chunk,
                        self._create_chunk_metadata([sub_chunk], i, i + 1)
                    ))
                continue
        ...
        if current_chunk:
            ...
            chunks.append((
                chunk_text,
                self._create_chunk_metadata(current_chunk, len(sentences) - len(current_chunk), len(sentences))
            ))
...
class OptimizedBatchProcessor:
    ...
    def get_processing_status(self) -> Dict[str, str]:
        ...
        return {
            doc_name: status
            for doc_name, status in self.processing_status.items()
        }
...
class ComparisonHistoryManager:
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if end_date and record.timestamp > end_date:
                            continue
                        if document_name:
                            if document_name not in [record.document_a, record.document_b]:
                                continue
                        if min_similarity and record.similarity_score < min_similarity:
                            continue
                        if max_similarity and record.similarity_score > max_similarity:
                            continue
                        if was_flagged is not None and record.was_flagged != was_flagged:
                            continue
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_documents": {
                r.document_a for r in records
            }.union({r.document_b for r in records}),
        }
...
def render_document_analysis_widget(doc_name: str, preprocessing_results: Dict[str, str]):
