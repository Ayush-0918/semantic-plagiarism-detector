    ...
    def get_export_history(self, limit: int = 50) -> List[Dict]:
        ...
        for job in self.export_history[-limit:]:
            history.append({
                ...
                "created_at": datetime.fromtimestamp(job.created_at).strftime("%Y-%m-%d %H:%M"),
                "file_size": f"{job.file_size / 1024:.1f} KB" if job.file_size else "N/A",
                "error": job.error
            })
        return history
    ...
    if st.button("📤 Generate Export", type="primary", use_container_width=True):
        with st.spinner("Generating export..."):
            ...
            if selected_template != "None":
                template = next((t for t in manager.templates if t.name == selected_template), None)
                if template:
            ...
            job = manager.export_data(
                ...
                filters={
                    "similarity": {"operator": "gt", "value": min_similarity}
                } if min_similarity > 0 else {},
...
def render_export_history(manager: ExportManager):
    ...
    completed_jobs = [j for j in manager.export_history if j.status == ExportStatus.COMPLETED]
...
def render_template_management(manager: ExportManager):
    ...
    with st.expander("➕ Create Template", expanded=False):
        ...
        with col2:
            ...
            template_fields = st.text_input("Fields (comma separated)", "doc_a,doc_b,similarity,status")
...
def render_integration_settings(manager: ExportManager):
    ...
    if selected_cloud != "None":
        ...
        with col1:
            ...
            credentials = st.text_area("Credentials (JSON)", placeholder='{"token": "xxx"}')
        with col2:
    ...
    webhook_events = st.multiselect(
        "Trigger Events",
        ["export_completed", "export_failed", "new_export"]
    )
