...
class PlagiarismReportGenerator:
    ...
    def _generate_pdf_report(self, report_data: ReportData) -> bytes:
        ...
        # Metadata
        story.append(Paragraph(f"Generated: {report_data.generated_at}", self.normal_style))
        story.append(Paragraph(f"Author: {self.config.author}", self.normal_style))
        if self.config.company:
            story.append(Paragraph(f"Company: {self.config.company}", self.normal_style))
        story.append(Spacer(1, 0.25 * inch))
        ...
        if report_data.flagged_pairs_list:
            ...
            for idx, pair in enumerate(pairs, 1):
                severity = "High" if pair.get('similarity', 0) > 0.90 else "Medium" if pair.get('similarity', 0) > 0.80 else "Low"
            ...
            pair_table = Table(pair_table_data, colWidths=[0.5*inch, 1.5*inch, 1.5*inch, 1*inch, 0.8*inch])
    ...
    def _generate_html_report(self, report_data: ReportData) -> bytes:
        ...
        if report_data.flagged_pairs_list:
            ...
            for idx, pair in enumerate(report_data.flagged_pairs_list[:self.config.max_pairs_to_show], 1):
                similarity = pair.get('similarity', 0)
                severity_class = "high" if similarity > 0.90 else "medium" if similarity > 0.80 else "low"
                html_content += f"""
                            <tr>
                                <td>{idx}</td>
    ...
    def _generate_json_report(self, report_data: ReportData) -> bytes:
        ...
        json_data = {
            ...
            "flagged_pairs": report_data.flagged_pairs_list[:self.config.max_pairs_to_show],
...
def _build_report_data_from_session() -> ReportData:
    ...
    # Calculate metrics
    total_pairs = len(doc_names) * (len(doc_names) - 1) // 2 if len(doc_names) > 1 else 0
