    ...
    def _get_active_sessions(self) -> int:
        ...
        try:
            ...
            if cache.is_available():
                ...
                for key in keys:
                    try:
                        session_id = key.split(b":")[3].decode() if isinstance(key, bytes) else key.split(":")[3]
                        last = cache.get(f"spd:v1:session:{session_id}:last_interaction")
...
class HealthChecker:
    ...
    def check_database() -> Dict[str, Any]:
        ...
        try:
            ...
            if CORPUS_DB_PATH.exists():
                ...
                result["details"].append(f"Corpus DB: {result['corpus_size_mb']:.1f} MB")
    ...
    def run_full_health_check() -> Dict[str, Any]:
        ...
        results["overall_status"] = "healthy" if all(critical_checks) else "degraded" if any(critical_checks) else "critical"
...
def render_system_health_dashboard():
    ...
    st.markdown(f"{status_color} **Overall Status:** {health['overall_status'].upper()}")
    ...
    with comp_col1:
        ...
        st.markdown(f"{status} Database: {'Healthy' if db['corpus_healthy'] else 'Unhealthy'}")
        if db["corpus_healthy"]:
            st.caption(f"  Corpus: {db['corpus_size_mb']:.1f} MB")
        
        redis = health["redis"]
        status = "🟢" if redis["healthy"] else "🔴"
        st.markdown(f"{status} Redis: {'Connected' if redis['healthy'] else 'Disconnected'}")
        if redis["healthy"]:
    ...
    with comp_col2:
        ...
        st.markdown(f"{status} Embedding Model: {'Loaded' if embedding['healthy'] else 'Failed'}")
        if embedding["healthy"]:
            st.caption(f"  Dimensions: {embedding['embedding_dim']}")
        
        faiss = health["faiss_index"]
        status = "🟢" if faiss["healthy"] else "🔴"
        st.markdown(f"{status} FAISS Index: {'Available' if faiss['healthy'] else 'Missing'}")
        if faiss["healthy"]:
...
def render_performance_charts(monitor: MonitoringEngine):
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=("CPU Usage", "Memory Usage", "Disk Usage", "Network I/O", "Active Sessions", "Processing Speed"),
    ...
    # CPU
    fig.add_trace(
        go.Scatter(x=timestamps, y=[m.cpu_percent for m in metrics],
                   name="CPU", line=dict(color="#ff6b6b", width=2)),
        row=1, col=1
    )
    ...
    # Memory
    fig.add_trace(
        go.Scatter(x=timestamps, y=[m.memory_percent for m in metrics],
                   name="Memory", line=dict(color="#4ecdc4", width=2)),
        row=1, col=2
    )
    ...
    # Disk
    fig.add_trace(
        go.Scatter(x=timestamps, y=[m.disk_usage for m in metrics],
                   name="Disk", line=dict(color="#ffe66d", width=2)),
        row=2, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=timestamps, y=net_sent,
                   name="Sent (MB)", line=dict(color="#a8e6cf", width=2)),
        row=2, col=2
    )
    fig.add_trace(
        go.Scatter(x=timestamps, y=net_recv,
                   name="Recv (MB)", line=dict(color="#ffd93d", width=2)),
        row=2, col=2
    )
    
    # Active Sessions
    fig.add_trace(
        go.Scatter(x=timestamps, y=[m.active_sessions for m in metrics],
                   name="Sessions", line=dict(color="#6c5ce7", width=2)),
        row=3, col=1
    )
    
    # Processing Speed
    fig.add_trace(
        go.Scatter(x=timestamps, y=[m.processing_speed for m in metrics],
                   name="Speed", line=dict(color="#fd79a8", width=2)),
        row=3, col=2
    )
...
def render_alerts_dashboard(monitor: MonitoringEngine):
    ...
    if active:
        ...
        for alert in active:
            ...
            with st.expander(f"{alert.severity.upper()}: {alert.message}", expanded=alert.severity == "critical"):
                st.markdown(f"**Severity:** {alert.severity.upper()}")
                st.markdown(f"**Time:** {datetime.fromtimestamp(alert.timestamp).strftime('%Y-%m-%d %H:%M:%S')}")
