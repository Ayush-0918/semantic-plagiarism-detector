...
class SmartSearchEngine:
    ...
    def search(self, query: str, top_k: int = 10, filters: Dict[str, Any] = None) -> List[SearchResult]:
        ...
        # Check cache
        cache_key = hashlib.md5(f"{query}_{top_k}_{json.dumps(filters or {})}".encode()).hexdigest()
        ...
        for key, doc_embedding in self.search_index.items():
            ...
            if similarity > 0.1:  # Minimum threshold
                ...
                result = SearchResult(
                    ...
                    matched_terms=self._extract_matched_terms(processed_query, registry_entry.get("text", "")),
                    context=self._extract_context(registry_entry.get("text", ""), processed_query)
                )
    ...
    def _calculate_similarity(self, query_vec: np.ndarray, doc_vec: np.ndarray) -> float:
        """Calculate similarity between query and document."""
        if len(query_vec) == 0 or len(doc_vec) == 0:
            return 0.0
        
        try:
            if isinstance(query_vec[0], (int, float)) and isinstance(doc_vec[0], (int, float)):
    ...
    def _apply_filters(self, results: List[SearchResult], filters: Dict) -> List[SearchResult]:
        ...
        if "min_similarity" in filters:
            filtered = [r for r in filtered if r.similarity_score >= filters["min_similarity"]]
    ...
    def get_search_stats(self) -> Dict[str, Any]:
        ...
        avg_results = sum(s.results_count for s in self.search_history) / len(self.search_history)
        avg_time = sum(s.execution_time for s in self.search_history) / len(self.search_history)
...
def render_smart_search_ui():
    ...
    with st.expander("🔧 Filters", expanded=False):
        ...
        with col3:
            document_filter = st.multiselect(
                "Filter Documents",
                options=st.session_state.get("doc_names", [])
            )
    ...
    if hasattr(st.session_state, "last_search_results") and st.session_state.last_search_results:
        ...
        if results:
            col2.metric("Max Similarity", f"{max(r.similarity_score for r in results):.2%}")
            col3.metric("Avg Similarity", f"{sum(r.similarity_score for r in results) / len(results):.2%}")
        ...
        for idx, result in enumerate(results, 1):
            with st.expander(
                ...
                with col1:
                    ...
                    st.info(result.chunk_text[:500] + "..." if len(result.chunk_text) > 500 else result.chunk_text)
                
                with col2:
                    st.markdown("**Details:**")
                    st.caption(f"Chunk: #{result.chunk_index}")
                    if result.matched_terms:
                        st.caption(f"Matched Terms: {', '.join(result.matched_terms[:5])}")
    ...
    if st.button("🔄 Detect Search Patterns", use_container_width=True):
        ...
        if patterns:
            ...
            for pattern_name, pattern in patterns.items():
                ...
                st.caption(f"Frequency: {pattern.frequency} | Type: {pattern.pattern_type}")
        else:
