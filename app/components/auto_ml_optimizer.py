...
class AutoMLOptimizer:
    ...
    def _load_history(self):
        ...
        try:
            history_path = Path(st.session_state.get("data_dir", ".")) / "optimization_history.json"
            if history_path.exists():
                with open(history_path, 'r') as f:
                    ...
                    self.best_threshold = max(self.threshold_history) if self.threshold_history else 0.75
    ...
    def _gradient_optimize(self) -> float:
        ...
        for delta in [-0.05, -0.02, 0.0, 0.02, 0.05]:
            candidate = max(self.config.min_threshold, 
    ...
    def _evaluate_threshold(self, threshold: float) -> OptimizationMetrics:
        ...
        precision = np.mean([m.precision for m in historical_metrics]) if historical_metrics else 0.5
        recall = np.mean([m.recall for m in historical_metrics]) if historical_metrics else 0.5
        ...
        return OptimizationMetrics(
            ...
            recall=recall,
            f1_score=f1,
            accuracy=np.mean([m.accuracy for m in historical_metrics]) if historical_metrics else 0.5,
            false_positives=np.sum([m.false_positives for m in historical_metrics]) if historical_metrics else 0,
            false_negatives=np.sum([m.false_negatives for m in historical_metrics]) if historical_metrics else 0,
            total_pairs=np.sum([m.total_pairs for m in historical_metrics]) if historical_metrics else 0,
            sample_size=len(historical_metrics)
        )
    
    def analyze_patterns(self, similarity_data: pd.DataFrame) -> Dict[str, PatternProfile]:
        """
    ...
    def get_optimal_threshold(self, context: Dict[str, Any] = None) -> float:
        ...
        if time.time() - self.last_optimization_time < self.config.optimization_interval:
            return self.best_threshold
        
        # Apply exploration (A/B testing)
        if random.random() < self.config.exploration_rate:
            # Explore: try a random threshold
            candidate = random.uniform(self.config.min_threshold, self.config.max_threshold)
...
class AutoOptimizationIntegration:
    ...
    def optimize_pipeline(self, similarity_data: pd.DataFrame, current_threshold: float) -> Tuple[float, Dict]:
        """
        ...
        context = {
            ...
            "avg_similarity": similarity_data.values.mean() if not similarity_data.empty else 0,
            "max_similarity": similarity_data.values.max() if not similarity_data.empty else 0,
        ...
        self.optimizer.add_metrics(OptimizationMetrics(
            timestamp=time.time(),
    ...
    def _calculate_pipeline_metrics(self, similarity_data: pd.DataFrame, threshold: float) -> Dict:
        """
    ...
    def get_optimization_dashboard_data(self) -> Dict:
        ...
        return {
            "summary": summary,
            "trends": trends,
            "recent_metrics": self.pipeline_metrics[-10:] if self.pipeline_metrics else [],
...
def render_auto_ml_dashboard():
    ...
    with st.expander("⚙️ Optimization Settings", expanded=False):
        ...
        with col2:
            exploration_rate = st.slider("Exploration Rate (A/B Testing)", 0.0, 0.3, 0.1, 0.01)
    ...
    if trends["f1_score"]:
        fig = make_subplots(rows=2, cols=2, subplot_titles=("F1 Score", "Precision", "Recall", "Accuracy"))
        
        # F1 Score
        fig.add_trace(
            go.Scatter(y=trends["f1_score"], name="F1 Score", line=dict(color="#2ecc71")),
            row=1, col=1
        )
        
        # Precision
        fig.add_trace(
            go.Scatter(y=trends["precision"], name="Precision", line=dict(color="#3498db")),
            row=1, col=2
        )
        ...
        # Accuracy
        fig.add_trace(
            go.Scatter(y=trends["accuracy"], name="Accuracy", line=dict(color="#f39c12")),
            row=2, col=2
        )
    ...
    if integration.pipeline_metrics:
        ...
        st.dataframe(
            df[["timestamp", "precision", "recall", "f1_score", "accuracy", "total_pairs"]].style.format({
                "precision": "{:.2%}",
                "recall": "{:.2%}",
                "f1_score": "{:.2%}",
                "accuracy": "{:.2%}"
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔄 Run Optimization Now", type="primary", use_container_width=True):
            with st.spinner("Optimizing..."):
                ...
                if sim_df is not None and not sim_df.empty:
                    ...
                    new_threshold, metrics = integration.optimize_pipeline(sim_df, current_threshold)
                    
                    st.success(f"✅ Optimization complete! New threshold: {new_threshold:.2%}")
    ...
    with col2:
        if st.button("📊 Export Optimization Data", use_container_width=True):
            data = {
                "summary": integration.optimizer.get_optimization_summary(),
                "metrics": [asdict(m) for m in integration.optimizer.metrics_history[-50:]],
                "feedback": [asdict(f) for f in integration.optimizer.feedback_history[-20:]],
        ...
        if history_path.exists():
            try:
                with open(history_path, 'r') as f:
                    ...
                    if thresholds:
                        st.session_state.auto_ml_optimizer.best_threshold = max(thresholds)
            except Exception:
                pass