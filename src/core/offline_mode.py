...
class OfflineConfig:
    ...
    def from_env(cls) -> "OfflineConfig":
        ...
        config.use_local_cache = os.getenv("OFFLINE_USE_LOCAL_CACHE", "true").lower() == "true"
        ...
        config.preload_models = os.getenv("OFFLINE_PRELOAD_MODELS", "true").lower() == "true"
        config.disable_telemetry = os.getenv("OFFLINE_DISABLE_TELEMETRY", "true").lower() == "true"
        config.disable_external_apis = os.getenv("OFFLINE_DISABLE_EXTERNAL_APIS", "true").lower() == "true"
        config.use_fallback_embedding = os.getenv("OFFLINE_USE_FALLBACK_EMBEDDING", "true").lower() == "true"
        
        try:
            config.max_cache_size_mb = int(os.getenv("OFFLINE_MAX_CACHE_SIZE_MB", "500"))
        except ValueError:
            config.max_cache_size_mb = 500
        
        config.auto_cleanup = os.getenv("OFFLINE_AUTO_CLEANUP", "true").lower() == "true"
        
        try:
            config.cleanup_interval_hours = int(os.getenv("OFFLINE_CLEANUP_INTERVAL_HOURS", "24"))
        except ValueError:
...
class OfflineModelManager:
    ...
    def get_model(self, model_name: str, load_func: Optional[callable] = None) -> Optional[Any]:
...
class OfflineModeManager:
    ...
    def __init__(self):
        ...
        self.model_manager = OfflineModelManager(self.config) if self.config.enabled else None
